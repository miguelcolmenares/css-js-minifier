		...
		if (fs.existsSync(newFileUri.fsPath)) {
			...
			if (openDocument) {
				...
				const editor = vscode.window.visibleTextEditors.find(
					(editor) => editor.document.uri.fsPath === newFileUri.fsPath
				);
				if (editor) {
					await vscode.window.showTextDocument(openDocument);
					await vscode.commands.executeCommand('workbench.action.closeActiveEditor');
...
suite.skip('CSS nth-child Test Suite', function () {
	...
	test('Minify CSS with nth-child selectors', async function () {
		...
		const expectedMinified =
			'.container:nth-child(odd){background-color:#fff;margin:10px}div:nth-child(odd){color:#00f;padding:5px}.item:nth-child(3nof.special){font-weight:700;border:1px solid red}p:nth-child(2n){text-align:center;font-size:14px}.menu-item:nth-child(2n+1of.active){display:block;opacity:.8}';
	...
	test('autoOpenNewFile setting - enabled', async function () {
		...
		if (newFileContent === tempContent) {
			...
			vscode.window.showInformationMessage(
				`Original length: ${tempContent.length}, Minified length: ${newFileContent.length}`
			);
		}
