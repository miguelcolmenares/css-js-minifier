...
- **Release process now produces six platform-specific `.vsix` files instead of a single "universal" build**, matching the [`vsce package --target <platform>` guidance from the VS Code Extension API docs](https://code.visualstudio.com/api/working-with-extensions/publishing-extension#platformspecific-extensions). Each `.vsix` is built on its own native GitHub Actions runner and attached to the corresponding GitHub Release automatically by the new [`.github/workflows/release.yml`](.github/workflows/release.yml). Marketplace serves each user the artifact matching their OS/CPU — no more accidental "one platform's bindings ship to everyone" (see the Fixed entry for #176). Publishing to Marketplace remains manual for now: the maintainer downloads the six `.vsix` files from the GH Release page and runs `npx vsce publish --packagePath` for each one; a follow-up will automate this once `VSCE_PAT` is configured as a repo secret.
