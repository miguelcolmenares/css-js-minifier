...
- **Markdown is linted by [markdownlint-cli2](https://github.com/DavidAnson/markdownlint-cli2) and no longer reformatted by Prettier.** `*.md` moves to `.prettierignore`: Prettier rewrites code spans and fenced content whose exact whitespace is the subject of the surrounding sentence — the issue templates document GitHub's title prefixes as `` `[Bug]: ` ``, where the trailing space is literal. `.markdownlint-cli2.jsonc` disables the rules whose `--fix` would silently change meaning (`MD029` renumbering, `MD007` re-indenting, `MD038` code-span spaces, `MD028` adjacent callouts) and records why for each. 208 findings across the 16 tracked files were resolved — 197 structural ones (blank lines around headings, lists and fences) applied with `--fix` and verified content-free by `git diff -w`, plus 11 untagged fences given an explicit `text` language.
- **`format:check` and `lint:md` now run in CI.** Neither had anywhere to run: `npm test` invokes ESLint through `pretest`, but `format:check` was never wired into a workflow at all — which is how three Markdown files sat unformatted without anyone finding out. A single `quality` job in [`master.yml`](.github/workflows/master.yml) runs both once on Ubuntu rather than three times across the OS matrix, and the Husky `pre-commit` hook gains the Markdown check alongside the existing ESLint one.

- **Minimum VS Code version raised to `1.136.0`** to match the `@types/vscode` bump to `^1.136.0`. `vsce` refuses to package when `@types/vscode` is newer than `engines.vscode` — it exits 1 with `@types/vscode ^1.136.0 greater than engines.vscode ^1.134.0`, which failed all six platform builds. The `test-vscode-minimum` workflow, the `VSCODE_VERSION` pin in [`.github/workflows/master.yml`](.github/workflows/master.yml), the `VSCODE_VERSION` fallback in [`.vscode-test.mjs`](.vscode-test.mjs), the README badge and `CONTRIBUTING.md` now all reference `1.136.0`. Note this raises the floor to the current stable line (1.136.1 at time of writing), so users still on 1.134 or 1.135 stop receiving updates until they upgrade. The floor moved twice within this unreleased cycle — 1.125.0 → 1.134.0 → 1.136.0 — and only the final value is recorded, since nothing shipped in between.
- **Bumped `@vscode/test-electron` from `^3.0.0` to `^3.1.0`** and restored `macos-latest` to the Insiders CI matrix in [`.github/workflows/test-vscode-insiders.yml`](.github/workflows/test-vscode-insiders.yml). v3.0.0 could not spawn Electron on darwin-arm64 after downloading the Insiders build (`ENOENT` on `Visual Studio Code - Insiders.app/Contents/MacOS/Electron`) because VS Code 1.110+ renamed that binary to `Code - Insiders` and — from 2026-07-20 — no longer shipped a compatibility symlink. The macOS runner was temporarily removed from the Insiders matrix as a stop-gap. v3.1.0 ships the upstream fix ([microsoft/vscode-test#350](https://github.com/microsoft/vscode-test/pull/350), authored by the maintainer of this extension, closing [microsoft/vscode-test#348](https://github.com/microsoft/vscode-test/issues/348) and [microsoft/vscode-test#349](https://github.com/microsoft/vscode-test/issues/349)): the executable is now resolved via `CFBundleExecutable` in `Info.plist`, with a tier-2 sole-regular-file scan of `Contents/MacOS/` and a legacy `Electron` fallback. macOS Insiders coverage is restored.
...
- **CRITICAL: Extension activation on Linux, Windows, and Intel Mac** ([#176](https://github.com/miguelcolmenares/css-js-minifier/issues/176), also root-causes [#118](https://github.com/miguelcolmenares/css-js-minifier/issues/118))
  - Since v1.3.0 the published `.vsix` on the Marketplace only bundled the `darwin-arm64` native bindings for `lightningcss` and `oxc-minify` (the two build platforms the maintainer used), and never bundled `detect-libc` (a runtime dependency of `lightningcss` on Linux). Every user on a non-Apple-Silicon machine hit `require('lightningcss') → Cannot find module 'detect-libc'` or `require('oxc-minify') → Cannot find native binding` during extension activation. Because both libraries are eagerly `require()`d at the top of `src/services/strategies/local*Minifier.ts`, the activation function threw before it could register any command — VS Code then reported "command not found" for every menu / keybinding invocation. This was silent (no toast, no error dialog); users had to open the Extension Host log to see anything.
  - The fix has two parts: (1) `detect-libc` is now declared as a direct `dependencies` entry in `package.json` and explicitly whitelisted in `.vscodeignore`, so `vsce` bundles it into every `.vsix`; and (2) the release pipeline moved from a single "universal" build produced on the maintainer's laptop to a per-platform build matrix that runs on GitHub Actions native runners (`macos-15-intel`, `macos-latest`, `ubuntu-latest`, `ubuntu-24.04-arm`, `windows-latest`, `windows-11-arm`). On each runner `npm ci` naturally installs the correct native `optionalDependencies` for that platform, `vsce package --target <t>` produces a platform-tagged `.vsix`, and a new `scripts/verify-vsix-activation.mjs` smoke test extracts the archive and confirms `require('lightningcss').transform(...)` and `require('oxc-minify').minifySync(...)` both work before uploading the artifact.
  - Also invalidates the previous root-cause attribution for #118 (the `navigator` global / `PendingMigrationError` hypothesis, which was Copilot speculation quoted by the reporter, not a verified diagnosis). Docker `linux/amd64` reproduction against a v1.3.2 `.vsix` produced the exact "cannot find module" errors documented above — matching #118's symptoms of silent activation failure and forced downgrade to v1.1.0.

### Fixed (continued from earlier drafts of 1.3.3)
...
- **Standalone Marketplace PAT verification workflow** ([`.github/workflows/verify-marketplace-auth.yml`](.github/workflows/verify-marketplace-auth.yml)) — a `workflow_dispatch` job that runs `vsce verify-pat miguel-colmenares` and reports the result in the workflow summary. Lets the maintainer confirm at any time that `VSCE_PAT` is present, unexpired, correctly scoped to `Marketplace (Manage)` for all accessible organizations, and matches the target publisher — without publishing anything or burning a version number. The full release workflow's `preflight` job runs the same check, so this standalone workflow is a lightweight smoke test to run after rotating the PAT.
- **Local Git hooks via Husky v9** (`.husky/pre-commit`, `.husky/commit-msg`, `.husky/pre-push`, installed automatically on `npm ci` through the new `prepare` script):
  - `pre-commit` runs `npm run lint` on `src/**`, and — when the commit modifies `package.json` `version` — requires `CHANGELOG.md` to be staged in the same commit with a matching `## [X.Y.Z]` heading. Mirrors the release workflow's preflight regex exactly, so a bump commit that passes locally is guaranteed to pass in CI.
  - `commit-msg` enforces Conventional Commits format (`feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `revert`) on the subject line, with auto-passthrough for `Merge …`, `Revert …`, `fixup! …`, and `squash! …` messages.
  - `pre-push` refuses to push any `refs/tags/v*`. Release tags must originate from the workflow — this prevents a maintainer from accidentally pushing a `v*` tag from their laptop and bypassing the PAT preflight check.
- **New `CONTRIBUTING.md`** — contributor onboarding covering local setup (Node 20+, npm 10+, VS Code 1.125+), development workflow with the VS Code watch tasks, expectations around cross-platform testing (the 6-platform CI matrix handles it, contributors do not need to own six machines), PR checklist, and a brief pointer to the maintainer-only release process.
- **New `AGENTS.md`** — agent-oriented reference covering the release invariants (tag immutability + PAT expiration), the dispatch-driven pipeline diagram, the git-hook table with escape hatches, an explanation of why no `pre-tag` or `pre-release` hook exists (Git does not dispatch one; `pre-commit` bump check + `pre-push` tag rejection cover the same ground), the cross-platform CI matrix, and a common-operations cheat sheet.
- **New "Release Flow" and "Local Git Hooks (Husky)" sections in [`.github/copilot-instructions.md`](.github/copilot-instructions.md)** — documents the new workflow-driven release flow and the Husky hook policy so AI agents working in this repository operate under the same constraints as human maintainers.
- **Branch protection ruleset for `master` (`Protect master`, id `19564892`)** — new repository ruleset that requires every change to `master` to arrive via pull request, blocks force-pushes and deletion, and gates merges on four status checks (`Test on ubuntu-latest`, `Test on macos-latest`, `Test on windows-latest`, `Analyze (javascript-typescript)` from CodeQL). `bypass_actors` is empty, so the protection applies to the owner as well — direct `git push origin master` is refused for everyone. The four required checks are the ones that always run on every PR to `master` regardless of the paths touched, so docs-only PRs are never blocked by "waiting for a check that never triggered". See [`AGENTS.md → Branch protection`](AGENTS.md#branch-protection-master) for the full rule-by-rule breakdown.

### Changed

- **Release process now produces six platform-specific `.vsix` files instead of a single "universal" build**, matching the [`vsce package --target <platform>` guidance from the VS Code Extension API docs](https://code.visualstudio.com/api/working-with-extensions/publishing-extension#platformspecific-extensions). Each `.vsix` is built on its own native GitHub Actions runner and attached to the corresponding GitHub Release automatically by [`.github/workflows/release.yml`](.github/workflows/release.yml). Marketplace serves each user the artifact matching their OS/CPU — no more accidental "one platform's bindings ship to everyone" (see the Fixed entry for #176).
- **Marketplace publishing is now fully automated via `workflow_dispatch`.** The old "push a `v*` tag → CI publishes" model was replaced by a dispatched release workflow whose first job (`preflight`) validates that the requested version matches `package.json` on `master`, that `CHANGELOG.md` has a corresponding `## [X.Y.Z]` heading, that no tag with the same name already exists, and — critically — that the `VSCE_PAT` secret is still valid via `vsce verify-pat miguel-colmenares` **before any tag or GitHub Release is created**. Only if preflight and the 6-platform build matrix both succeed does a subsequent job create the annotated tag, push it, cut the GitHub Release, and finally run `vsce publish --packagePath dist/*.vsix`. Combined with the repository's `refs/tags/v*` immutability ruleset (blocks `deletion` and `non_fast_forward`), this makes it impossible to burn a version number on GitHub while failing to reach the Marketplace when the Azure DevOps PAT silently expires. The full runbook is in [`.github/instructions/publish-update-extension.instructions.md`](.github/instructions/publish-update-extension.instructions.md).
