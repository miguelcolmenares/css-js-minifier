...
- **Minimum VS Code version raised to `1.134.0`** to match the `@types/vscode` bump to `^1.134.0`. `vsce` refuses to package when `@types/vscode` is newer than `engines.vscode`. The `test-vscode-minimum` workflow, the `VSCODE_VERSION` pin in [`.github/workflows/master.yml`](.github/workflows/master.yml), the `VSCODE_VERSION` fallback in [`.vscode-test.mjs`](.vscode-test.mjs), the README badge and `CONTRIBUTING.md` now all reference `1.134.0`.
